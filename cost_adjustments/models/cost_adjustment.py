    ...
    def _compute_count(self):
        ...
        for val in self:
            ...
            if valuation_layers:
                ...
                #         x: x.real_date < inv_layer[0].real_date and x.value > 0)
                # if not in_layers:
                #     stock_out_after_real = stock_out_after_real - in_layers
                # stock_out_after_real = stock_out_after_real - (valuation_layers - wrong_in_layer)
            ...
            if val.count == 0:
                val.from_date = False
                val.active = False
                # return_layers = valuation_layers_all.filtered(
                #     lambda
                #         x: x.product_id == val.product_id and x.stock_move_id.origin_returned_move_id != False).sorted(
                #     key=lambda x: x.real_date)
                # print('return_layers', return_layers)
                # issue_layer = self.env['stock.valuation.layer']
                # for svl in return_layers:
                #     print('svl.stock_move_id.origin_returned_move_id', svl.stock_move_id.origin_returned_move_id)
                ...
                #             return_layers = return_layers - svl
                #         else:
                #             issue_layer+=svl
                #
                # if issue_layer:
                #     print('issue_layer', issue_layer[0], issue_layer[0])
                #     val.from_date = issue_layer[0].real_date
                #     val.count = len(issue_layer)
                ...
                #     print('to_be_adjusted', to_be_adjusted)
                #     if to_be_adjusted:
