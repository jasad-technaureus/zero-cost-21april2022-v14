...
class PurchaseBookWizard(models.TransientModel):
    ...
    purchase_book_file = fields.Binary('Purchase Report')
    ...
    def print_xls_report(self):
        new_file = self.env['purchase.sale.book'].excel_report_generate('purchase', self.from_date, self.to_date)
        output = base64.encodebytes(new_file)
        self.purchase_book_file = output
        self.file_name = 'Purchase Book.xlsx'
        self.report_printed = True
        return {
            'view_mode': 'form',
            'res_id': self.id,
            'res_model': 'purchase.book.wizard',
            'view_type': 'form',
